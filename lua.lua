...
local function FarmScreen(title)
	...
	return self
		...
		local Window = Fluent:CreateWindow({
			...
			Author = "Fisch Script | discord.gg/thanhub",
			Folder = "ThanHub",
...
function AutoEnd()
	...
	ReplicatedStorage.Packages.RemotePromise.Remotes.C_ActivateObject:FireServer(workspace.RuntimeItems.RevolverAmmo)
	task.wait(0.2)
	ReplicatedStorage.Packages.RemotePromise.Remotes.C_ActivateObject:FireServer(workspace.RuntimeItems.RevolverAmmo)
    ...
    local function processBond(bond)
        if bond and bond:FindFirstChild("Part") then
            ...
            if bondPart:IsA("BasePart") then
                ...
                Chair.Seat.CFrame = bondCFrame
                task.wait(0.1)
                local activateRemote = ReplicatedStorage.Packages.RemotePromise.Remotes.C_ActivateObject
                pcall(function()
        ...
        if ensureChairMovement(currentSeat) then
            ...
            for _, item in pairs(runtimeItems:GetChildren()) do
                if item.Name == "Chair" and item ~= (currentSeat and currentSeat.Parent) then
                    local otherSeat = item:FindFirstChildOfClass("Seat") or item:FindFirstChildOfClass("VehicleSeat")
                    if otherSeat then
